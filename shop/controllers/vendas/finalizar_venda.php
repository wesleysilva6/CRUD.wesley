...
session_start();
    ...
    $funcionario  = $_SESSION['funcionario'] ?? null;
    ...
    if (empty($_SESSION['venda']) || $total <= 0) {
        header('Location: ../../views/vendas.php?erro=venda_vazia');
        exit;
    }

    $stmt = $conn->prepare("INSERT INTO vendas (cliente, telefone, funcionario_id, total, realizada_em) VALUES (?, ?, ?, ?, ?)");
    $stmt->bind_param('ssids', $nome_cliente, $tel_cliente, $funcionario, $total, $realizada_em);
    $stmt->execute();
    ...
    foreach ($_SESSION['venda'] as $produto_id => $item) {
        ...
        $stmt1->close();
        unset($_SESSION['venda'], $_SESSION['cliente'], $_SESSION['telefone'], $_SESSION['funcionario']);
